"""Data models for calendar events and requests."""
...
def convert_datetime(v):
    ...
    # Handle EventKit NSDate objects
    ...
    # Let Pydantic handle unknown types
    return v
...
class RecurrenceRule(BaseModel):
    ...
    def validate_end_conditions(self) -> Self:
        """Ensure only one end condition is set."""
    ...
    def to_ek_recurrence(self) -> EKRecurrenceRule:
        """Convert to EventKit recurrence rule."""
...
@dataclass
class Event:
    """Calendar event with all metadata."""
    ...
    def to_summary_string(self) -> str:
        ...
        return f"{time_str}: {self.title}{location_str} [{self.duration_minutes}min]"
